...
describe('App component', () => {
  ...
  test('displays error message when fetch fails', async () => {
    fetchMock.mockResolvedValue(new Error('Failed to fetch posts'))
    render(<App/>)
    const error = await screen.findByText(/Failed to fetch posts/i)
  ...
  test('displays loading message while fetching posts', async () => {
    fetchMock.mockImplementationOnce(() => new Promise((resolve) => setTimeout(() =>
      resolve({ok: true, json: () => Promise.resolve([])}), 1000)))
    render(<App/>)
